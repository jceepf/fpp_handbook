...
\multirow{3}{*}{ \refstepcounter{mytable} \themytable}
 & & \\
 & Transforming a vector field with a vector field& \href{\tb z_vf_Lie_sub.f90}{F=exp_ad( F,F')}\\
  & & \\
 \hline
\multirow{3}{*}{ \refstepcounter{mytable} \themytable}
 & & \\
...
%|<c$4Q^:$f}: -],D}& b!( b"0 b#8 b$@ b%H b&P!WW}]|[
...
To understand these three instantiations of the \vn{real8/complex_8} types, we propose the  code \vn{z_track_real_8.f90}. Consider these fragments from the program  \vn{z_track_real_8.f90}:
...
Before discussing the \vn{c_damap}, we need to understand a few parameters that are crucial to that map and are global in AP. We can show a code fragment that prints them in  particularly complex cases.  This is from the main program \vn{z_track_c_.f90}. 
...
\end{code}
  ...
  See program \vn{z_track_normal_tpsa.f90} for a simple call to the normal form.
...
An example of this useful call is in program  \vn{z_track_normal_fact.f90}. This example computes  the so-called beta function in two different ways using $a_1$,  First if defines $a_1$ as the coefficient of $p_x^2$ in the quadratic invariant. Secondly it defines it has the coefficient of $J_1$ in the computation of $<x^2>$. In the Ripken formalism, these are the same functions.
...
In the above code fragment, which is in \vn{z_track_normal_fact.f90}, there three important steps.  
