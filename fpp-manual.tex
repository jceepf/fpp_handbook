...
The name PTC is commenly used to designate a code written by Etienne Forest with the help of Frank Schmidt and later David Sagan. However,  it is made of several parts. In particular, it is linked with the overloaded TPSA package of Martin Berz, called polymorphic package, (PP), via the creation of a derived type called \vn{real_8}.  It should become obvious to the reader that {\bf any} tracking code using PP is automatically ``a polymorphic tracking code'' or a ``PTC''.  In this manual, from now own,  we used PTC to refer specifically to the code you download from Cornell with Bmad---Forest's code.

PTC is an object oriented package, open source, made of 3 separate entities. It stands for ``Polymorphic Tracking Code''. 

\begin{enumerate}
\item The tracking code itself (call it TC)  is where the physics of PTC is located. It is simply an integrator pushing the particles (phase space, spin and modulation) through magnets.  Via the magic of PP, a Taylor map can be produced if wanted out of TC.
\item\label{it:fpp} The ``Polymorphic Package'' (the PP of FPP)   deals  with the production of Taylor series. It is completely void of physics.
\item\label{it:ap}  The Analysis Package (AP)  analyses physically sensible Taylors maps: Taylor maps that approximate the tracking of the code. Spin and magnet modulation can also be analyzed. FPP is  the combination
of items \ref{it:fpp} and \ref{it:ap}. The interaction with TC is described in section (\sref{s:extmap} ).
\end{enumerate}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=1.2\textwidth]{ptcfppe.pdf}
  \caption{
...
%|$^:"z^1_,K$^N)!# b'4$^z^0_}& b!( b"0 b#8 b$@ b%H b&P!WW}_:!,H|
%|:#&c0  D:"&c0!*,M$^z^0_:!,I : ;8/<:";bP8eq,Zconcatt: ;8/=:!;bP8 |
%|: -;}& b!( b"0 b#8 b$@ b%H b&P!WW}]|[
...
%]|Expr|[#b @`b___})b I# b'4" Chicago^: ;bP8&c0!*-="!Times|:!t|
%|""Helvetica|:",F,]<c!$1(#$^"#*|:#m)!# b'4$^w^0_|
%|}& b!( b"0 b#8 b$@ b%H b&P!WW}_"$Symbol^:$&c0  .P$^:#&c0!*n)!# b'4|
%|$^z^0_}& b!( b"0 b#8 b$@ b%H b&P!WW}_}}<c!$1^:$&c0  D}: ;bP;&c0!*/"|
%|:";bP8 ,F,]$^:#w^1_,K$^M)!# b'4$^w^0_}& b!( b"0 b#8 b$@ b%H b&P!WW}|
...
%|^0_}& b!( b"0 b#8 b$@ b%H b&P!WW}_:",H$^:#N)!# b'4$^z^0_|
%|}& b!( b"0 b#8 b$@ b%H b&P!WW}_:",H:$&c0  D:#&c0!*,M$^z^0_:",I,K|
%|$^:#z^1_,M$^w^0_:",I:# : ;8/<:#;bP8eq,Zconcattot: ;8/=;bP8-;|
%|}& b!( b"0 b#8 b$@ b%H b&P!WW}]|[
...
The code for this section is located at \vn{z_track_map_code.f90}. If you run the code, you can select case 1,2,3 or 4 which correspond respectively to sections 
...
\end{code}
 ...
 The code for this section is located at \vn{z_track_normal_tpsa.f90}. 
...
{\color{h}These are used  via the polymorphic types \vn{real_8} and \vn{complex_8} in the tracking code TC.}
%--------------------------------------------------------------------------------------------------------
...
\begin{enumerate}
\item Mode ignorant of accelerator phase space maps. In that case, FPP does not know what a normal form is. Dimension of \vn{c_damap} must be specified by the user. You are on your own. This is not recommended if you have an even dimension phase space that represents the dynamics of beams.
...
lagrange_map_ap%n=nv   ! type c_damap
call alloc(lagrange_map_ap)  ! type c_damap 
\end{code}

This initialization of TPSA and AP creates \vn{nv} variables of order \vn{no}: there is no mention of maps. Thus the number of dimension of  \vn{lagrange_map_ap}, a  \vn{c_damap}, must be specified in \vn{lagrange_map_ap%n}. Here it has \vn{nv}  variables. 
This number can be from  \vn{lagrange_map_ap%n=1} to \vn{lagrange_map_ap%n=nv}.
